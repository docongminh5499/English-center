					...
					<Grid>
						...
						<Grid.Col span={6}>
							<Button 
								...
								style={{float: "right"}}
								onClick={() =>{ 
									setDeleteQuestions((current: any) => [...current, item.id])
  ...
  const handleModifyExercise = async () =>{
    try {
      ...
      props.setListExercises((listExercise: Exercise[]) => {
        return listExercise.map((exercise: Exercise, index: number) => exercise.id === exerciseResponse.id ? exerciseResponse : exercise);
      })
