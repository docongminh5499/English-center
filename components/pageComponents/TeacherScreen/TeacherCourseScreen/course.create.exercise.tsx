import { Box, Button, Container, Divider, FileButton, Grid, Group, Loader, Modal, MultiSelect, Text, Textarea, TextInput, Title } from "@mantine/core";
...
const CourseCreateExercise = (props: any) => {
  console.log(props);
  const now = new Date();
  const [timeDoExercise, setTimeDoExercise] = useState<[Date, Date]>([now, now]);
  const [dateDoExercise, setDateDoExercise] = useState<DateRangePickerValue>([now, now]);
  ...
  const basicInfo = useForm({
    initialValues: {
      nameExercise: "",
      maxTime: 1,
      dateDoExercise: [now, now],
      timeDoExercise: [now, now],
    },
