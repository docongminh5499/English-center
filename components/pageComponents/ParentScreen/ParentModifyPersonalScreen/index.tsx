...
const ParentModifyPersonalScreen = (props: any) => {
  ...
  const [currentUserParent, setCurrentUserParent] = useState(props.userParent);
  // Personal info state
  ...
  const updatePersonalInfoForm = useForm({
    initialValues: {
      fullName: currentUserParent?.user.fullName,
      gender: currentUserParent?.user.sex,
      dateOfBirth: moment(currentUserParent?.user.dateOfBirth).utcOffset(TimeZoneOffset).toDate(),
      address: currentUserParent?.user.address,
      email: currentUserParent?.user.email,
      phone: currentUserParent?.user.phone,
      avatar: null
    },
  ...
  const onSave = useCallback(async (data: any) => {
    setIsSaving(true);
    if (currentUserParent === null) {
      setIsSaveModalOpen(false);
      setIsSaving(false);
      toast.error("Cập nhật thông tin thất bại");
      return;
    }
    currentUserParent.user.fullName = data.fullName;
    currentUserParent.user.dateOfBirth = data.dateOfBirth;
    currentUserParent.user.sex = data.gender;
    currentUserParent.user.address = data.address;
    currentUserParent.user.email = data.email;
    currentUserParent.user.phone = data.phone;

    const obUserParent: { [key: string]: any; } = Object.assign({}, currentUserParent);
    try {
      const formData = new FormData();
      formData.append("avatar", data['avatar']);
      formData.append("userParent", JSON.stringify(obUserParent));
      const responses: any = await API.post(
        Url.parents.modifyPersonalInformation, formData, {
        headers: {
      ...
      if (responses.success) {
        ...
        await router.push("/parent/personal");
  ...
  }, [currentUserParent, authState.token, router, toast])
            ...
            <form
              ...
              <Container style={{
                ...
                <Image
                  ...
                  radius="md"
                  src={getAvatarImageUrl(currentUserParent?.user.avatar)}
                  alt="Hình đại diện"
                />
                <FileInput
                  accept="image/*"
...
export default ParentModifyPersonalScreen;