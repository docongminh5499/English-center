import { Badge, Box, Card, Checkbox, Container, Grid, Image, Input, Pagination, SimpleGrid, Space, Text } from "@mantine/core";
...
import { CourseStatus, StudentConstants, TimeZoneOffset, Url, UserRole } from "../../../../helpers/constants";
...
const ParentCourseScreen = (props: any) => {
  ...
  const [maxPage, setMaxPage] = useState(
    Math.ceil(
      (pageable?.total || 1) /
      (pageable?.limit || StudentConstants.limitCourse)
    )
  );
    ...
    const didMountFunc = async () => {
      try {
        ...
        if(selectedStudent === null)
          return;
        const exerciseResponse = await API.get(Url.parents.getPageableStudentCourses, {
												...
												return (
													<Card
														...
														onClick={() =>
															...
															query: {studentId: selectedStudent?.user.id}
															})
														}
													>
														<Card.Section>
