...
import { EmployeeConstants, TimeZoneOffset, Url } from "../../../../helpers/constants";
...
const EmployeeSalaryScreen = (props: IProps) => {
  ...
  const [maxPage, setMaxPage] = useState(Math.ceil(props.salaries.total / EmployeeConstants.limitSalary));
  ...
  const getSalaries = useCallback(async (limit: number,
    ...
    return await API.post(Url.employees.getSalaries, {
  ...
  const querySalaries = useCallback(async () => {
    try {
      setLoading(true);
      setError(false);
      const responses = await getSalaries(EmployeeConstants.limitSalary, 0, date[0], date[1]);
      setCurrentPage(1);
