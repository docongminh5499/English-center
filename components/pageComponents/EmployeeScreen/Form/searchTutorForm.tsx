import { Container, Group, Select, TextInput, Title, Text, ScrollArea, SimpleGrid, Avatar } from "@mantine/core";
...
const SearchTutorForm = (props: IProps) => {
  ...
  const onChangeBranch = useCallback(async () => {
    ...
    try {
      ...
      const tutors = await API.post(Url.employees.getAvailableTutors, {
  ...
  const didMountFunction = useCallback(async () => {
    ...
    try {
      ...
      const [branches, tutors] = await Promise.all([
        ...
        API.post(Url.employees.getAvailableTutors, {
          token: authState.token,
          branchId: branchId,
          beginingDate: props.beginingDate,
          shiftIds: props.shiftIds,
