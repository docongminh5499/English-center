...
const TutorModifyPersonalScreen = (props: IProps) => {
  const isLargeTablet = useMediaQuery('(max-width: 1024px)');
  const isMobile = useMediaQuery('(max-width: 480px)');
  const router = useRouter();
  const [authState, authAction] = useAuth();
  const [didMount, setDidMount] = useState(false);
  const [currentUserTutor, setCurrentUserTutor] = useState(props.userTutor);
  ...
  const updatePersonalInfoForm = useForm({
    initialValues: {
      fullName: currentUserTutor?.worker.user.fullName,
      gender: currentUserTutor?.worker.user.sex,
      dateOfBirth: moment(currentUserTutor?.worker.user.dateOfBirth).utcOffset(TimeZoneOffset).toDate(),
      passport: currentUserTutor?.worker.passport,
      nation: currentUserTutor?.worker.nation,
      homeTown: currentUserTutor?.worker.homeTown,
      address: currentUserTutor?.worker.user.address,
      email: currentUserTutor?.worker.user.email,
      phone: currentUserTutor?.worker.user.phone,
      avatar: null
    },
  ...
  const onSave = useCallback(async (data: any) => {
    setIsSaving(true);
    if (currentUserTutor === null) {
      setIsSaveModalOpen(false);
      setIsSaving(false);
      toast.error("Cập nhật thông tin thất bại");
      return;
    }
    currentUserTutor.worker.user.fullName = data.fullName;
    currentUserTutor.worker.user.dateOfBirth = data.dateOfBirth;
    currentUserTutor.worker.user.sex = data.gender;
    currentUserTutor.worker.passport = data.passport;
    currentUserTutor.worker.nation = data.nation;
    currentUserTutor.worker.homeTown = data.homeTown;
    currentUserTutor.worker.user.address = data.address;
    currentUserTutor.worker.user.email = data.email;
    currentUserTutor.worker.user.phone = data.phone;

    const obUserTutor: { [key: string]: any; } = Object.assign({}, currentUserTutor);
    try {
      const formData = new FormData();
      formData.append("avatar", data['avatar']);
      formData.append("userTutor", JSON.stringify(obUserTutor));
      const responses: any = await API.post(
        Url.tutors.modifyPersonalInformation, formData, {
        headers: {
      ...
      if (responses.success) {
        ...
        await router.push("/tutor/personal");
  ...
  }, [currentUserTutor, authState.token, router, toast])
          ...
          <form
            ...
            <Container style={{
              ...
              <Image
                ...
                radius="md"
                src={getAvatarImageUrl(currentUserTutor?.worker.user.avatar)}
                alt="Hình đại diện"
              />
              <FileInput
                accept="image/*"
...
export default TutorModifyPersonalScreen;