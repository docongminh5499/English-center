...
import { CourseStatus, TimeZoneOffset, TutorConstants, Url, UserRole } from "../../../../helpers/constants";
...
import styles from "./tutor.course.module.css";
...
const TutorCourseScreen = (props: IProps) => {
  ...
  const [maxPage, setMaxPage] = useState(Math.ceil((
    props.pageable?.total || 1) / (props.pageable?.limit || TutorConstants.limitCourse)));
  ...
  const onClickPaginationPage = useCallback(async (page: number,
    ...
    try {
      ...
      await getCourse(
        TutorConstants.limitCourse,
        (page - 1) * TutorConstants.limitCourse,
  ...
  return (
    <>
      ...
      <div className={styles.tutorCoursePage}>
                    ...
                    {course[key].map((courseInfo: Course) => {
                      return (
                        <Card
                          ...
                          onClick={() => router.push("/tutor/course/" + courseInfo.slug)}>
                          <Card.Section>
                            <Image
...
export default TutorCourseScreen;
