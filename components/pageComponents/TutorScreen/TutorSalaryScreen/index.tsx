...
const TutorSalaryScreen = (props: IProps) => {
  ...
  const [maxPage, setMaxPage] = useState(Math.ceil(props.salaries.total / TutorConstants.limitSalary));
  ...
  const getSalaries = useCallback(async (limit: number,
    ...
    return await API.post(Url.tutors.getSalaries, {
  ...
  const querySalaries = useCallback(async () => {
    try {
      setLoading(true);
      setError(false);
      const responses = await getSalaries(TutorConstants.limitSalary, 0, date[0], date[1]);
      setCurrentPage(1);
      setTotal(responses.total);
      setMaxPage(Math.ceil(responses.total / TutorConstants.limitSalary));
  ...
  const onClickPaginationPage = useCallback(async (page: number) => {
    try {
      ...
      const responses = await getSalaries(
        TutorConstants.limitSalary,
        (page - 1) * TutorConstants.limitSalary,
        date[0], date[1]
      );
      setTotal(responses.total);
      setMaxPage(Math.ceil(responses.total / TutorConstants.limitSalary));
...
export default TutorSalaryScreen;